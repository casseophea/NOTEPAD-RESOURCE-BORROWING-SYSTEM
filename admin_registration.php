...
<div class="container">
  ...
  <div class="right">
    ...
    <form method="POST" action="admin_registration.php" id="adminRegisterForm">
      ...
      <div class="role-brgy-row">
        <label>Role in Barangay <span class="required">*</span></label>
        <select class="role-type" name="barangay_role" required>
          <option value="" disabled selected>Role</option>
          <option value="Barangay Captain" <?php echo (($_POST['barangay_role'] ?? '') === 'Barangay Captain') ? 'selected' : ''; ?>>Barangay Captain</option>
          <option value="Barangay Councilor" <?php echo (($_POST['barangay_role'] ?? '') === 'Barangay Councilor') ? 'selected' : ''; ?>>Barangay Councilor</option>
          <option value="Barangay Tanod" <?php echo (($_POST['barangay_role'] ?? '') === 'Barangay Tanod') ? 'selected' : ''; ?>>Barangay Tanod</option>
        </select>
      </div>
...
<script>
  ...
  function autoSaveForm() {
    try {
      const fields = {
        ...
        barangay_role: document.querySelector('select[name="barangay_role"]').value
      };
      sessionStorage.setItem('admin_register_form_state', JSON.stringify(fields));
    } catch (e) {
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    try {
      const saved = sessionStorage.getItem('admin_register_form_state');
      if (saved) {
        ...
        if (fields.barangay_role !== undefined) document.querySelector('select[name="barangay_role"]').value = fields.barangay_role;
      }
    } catch (e) {
  ...
  document.getElementById('adminRegisterForm').addEventListener('submit', () => {
    ...
    sessionStorage.removeItem('admin_register_form_state');
  });
  ...
  function saveAndUpload() {
    autoSaveForm();
    window.location.href = 'upload_id.php?from=admin_registration';
  }
</script>
</body>
</html>
